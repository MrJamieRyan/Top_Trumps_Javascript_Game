<template>
  <div :class="winningPlayer === 'player-one' || winningPlayer === 'bothCardsShowing' ? 'card-up' : 'card-down'" >
    ...
    <p :class="winningPlayer === 'player-two'|| winningPlayer === 'bothCardsShowing' ? 'no-text' : ''" v-on:click="handleClick(key.toLowerCase())" v-for="(value, key) in cardWithoutName"> {{key}}: {{value}} </p>
